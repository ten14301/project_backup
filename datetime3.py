...
def wait_an_event_thread(trigger):
                while not trigger.wait(10800):
                    print ("3 hour has passed")
                    now = datetime.now()
                    print("now =", now)
                    dt_string3 = now.strftime("%d/%m/%Y %H:%M:%S")
                    print("date and time =", dt_string3)
                    cur = conn.cursor() 
                    cur.execute("update dtmoisensor10 set datet = (%s)",(dt_string3, ))
                    cur.execute("update dtmoisensor20 set datet = (%s)",(dt_string3, ))
                    cur.execute("update dtmoisensor30 set datet = (%s)",(dt_string3, ))
